...
public class HttpUtils1 {
    ...
    public static HttpResponse call(CaseInfo1 caseInfo1, Map<String,String> headers)  {
        String method = caseInfo1.getMethod();
        String contentType = caseInfo1.getContentType();
        //如果请求方式是post
        try {
            if ("post".equalsIgnoreCase(method)) {
                //如果参数类型是json
                if ("json".equalsIgnoreCase(contentType)) {
                    return HttpUtils1.jsonpost(caseInfo1.getUrl(), caseInfo1.getParams(),headers);
                    //如果参数类型是form
                } else if ("form".equalsIgnoreCase(contentType)) {
                    String params = json2KeyValue(caseInfo1.getParams());
                    return HttpUtils1.formpost(caseInfo1.getUrl(), params,headers);
                } else {
                    System.out.println("没有发送http请求" + caseInfo1);
                }
                //如果请求方式是get
            } else if ("get".equalsIgnoreCase(method)) {
                return HttpUtils1.get(caseInfo1.getUrl(),headers);
                //如果请求方式是patch
            } else if ("patch".equalsIgnoreCase(method)) {
                return HttpUtils1.patch(caseInfo1.getUrl(), caseInfo1.getParams(),headers);
            } else {
                System.out.println("没有发送http请求" + caseInfo1);
            }
        }catch (Exception e){
